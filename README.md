...
меткой. Метка не украшение: прокси без меток Prowlarr не применяет вовсе.
Измерено — без метки запрос уходит напрямую и получает `403` за две секунды,
с меткой идёт через FlareSolverr и доходит до сайта за тридцать. Сам индексер
помечать не нужно, поэтому человек добавляет сайт обычным образом, ни о чём
не зная.

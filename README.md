...
**Человек вводит один логин и пароль.** Программа создаёт с ними аккаунт Jellyfin,
две медиатеки и вход в Jellyseerr. Технический пароль qBittorrent генерируется
отдельно и автоматически передаётся Sonarr и Radarr.
...
Один скачанный файл разворачивает на Linux медиастек из семи сервисов, связывает их
через API и показывает состояние без погружения пользователя в Kubernetes. Каждый релиз
проходит проверку на настоящем k3s до публикации.

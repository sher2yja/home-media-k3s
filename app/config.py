...
# одного номера, поэтому занятость этого порта проверяется до установки, а сам
# Traefik новым кластерам больше не ставится (--disable=traefik в K3S_EXEC).
GRAFANA = Service(
    key="grafana",
    title="Grafana",
    port=30030,
    ...
    # True, хотя Grafana и не медиасервис. Группы на вкладке делятся не по
    # важности, а по вопросу «это открывают или это работает само». Sonarr и
    # Prowlarr работают сами, а графики именно открывают — ради них полный
    # профиль и выбирают. В «Служебном» кнопка на дашборд лежала под заголовком
    # «работает само», то есть ровно там, куда не смотрят.
    user_facing=True,
